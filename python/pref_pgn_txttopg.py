...
--建立索引
CREATE INDEX v6_time_pref_pgn_utf_wgs84_geoserver_index ON v6_time_pref_pgn_utf_wgs84_geoserver USING 
btree(gid);
--表说明
...
f=open(r'c:\Users\zhou0\Desktop\MyGeoserverProject\data\V6 Time Series Prefecture Polygons\v6_time_pref_pgn_utf_wgs84\v6_time_pref_pgn_utf_wgs84.txt','r',encoding='utf-8')
# 按行读入txt
flines=f.readlines()
for line in flines:
    # 去掉干扰，切词
    ...
    geom='null'
    if (abbrlist[23]!=''):
        if(abbrlist[23].find('MULTI')>-1):
            polylist=abbrlist[23].replace('MULTIPOLYGON (((','').replace(')))','').split(')),((')
            for poly in polylist:
                geom="st_geomfromtext('POLYGON ((" + poly + "))',4326)"
                # 拼接sql语句
                sqltxt = "INSERT INTO v6_time_pref_pgn_utf_wgs84_geoserver(" \
                         ...
                         "beg_chg_ty,end_chg_ty,geometry) VALUES(" + name_py + "," + name_ch + "," + name_ft + "," + x_coor + "," + y_coor + "," \
